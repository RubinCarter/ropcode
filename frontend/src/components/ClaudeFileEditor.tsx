...
/**
 * ClaudeFileEditor component for editing project-specific Memory files
 * 
 * @example
 * <ClaudeFileEditor 
 *   file={claudeMdFile} 
...
export const ClaudeFileEditor: React.FC<ClaudeFileEditorProps> = ({
  ...
  const handleSave = async () => {
    try {
      ...
      setToast({ message: t("markdown.saveSuccess"), type: "success" });
    } catch (err) {
      console.error("Failed to save file:", err);
      setError(t("markdown.saveError"));
      setToast({ message: t("markdown.saveError"), type: "error" });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleBack = () => {
    if (hasChanges) {
      const confirmLeave = window.confirm(t("markdown.unsavedChanges"));
