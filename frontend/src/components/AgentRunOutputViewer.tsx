...
export function AgentRunOutputViewer({
  ...
  const {
    ...
    updateVisibleRange: _updateVisibleRange,
    appendMessage: appendWindowMessage,
    isLoading: _windowLoading
  } = useMessageWindow({
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        appendWindowMessage(stopMessage);
  ...
  useEffect(() => {
    ...
    // useMessageWindow handles initial loading
    // Just set up live listeners if needed
    loadOutput();
  }, [run?.id]);
  ...
  const displayableMessages = useMemo(
    () => getDisplayableMessages(messages, subagentProgress.subagentMessageIndexes).messages,
