...
export const getClaudeSyntaxTheme = (theme: ThemeMode): any => {
  // Map 'system' to 'gray' or 'light' based on system preference
  // In practice, components should detect system theme and pass the actual theme
  let effectiveTheme = theme;
  if (theme === 'system') {
    // Use system preference: dark mode → gray, light mode → light
    const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
    effectiveTheme = prefersDark ? 'gray' : 'light';
  }
  ...
  const colors = themes[effectiveTheme as keyof typeof themes] || themes.dark;
