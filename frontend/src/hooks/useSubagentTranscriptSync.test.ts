...
test('live subagent renderers use shared transcript sync and canonical merge point', async () => {
  ...
  assert.match(agentRunOutputViewerSource, /getDisplayableMessages\(messages, subagentProgress\.subagentMessageIndexes\)\.messages/);
  assert.doesNotMatch(agentRunOutputViewerSource, /isSubagentEnvelopeMessage\(message\)/);
  assert.match(agentRunViewSource, /buildSubagentProgress\(messages, subagentTranscripts\)/);
  assert.match(agentRunViewSource, /getDisplayableMessages\(messages, subagentProgress\.subagentMessageIndexes\)\.messages/);
  assert.doesNotMatch(agentRunViewSource, /isSubagentEnvelopeMessage\(message\)/);
  assert.match(sessionOutputViewerSource, /getDisplayableMessages\(messages, subagentProgress\.subagentMessageIndexes\)\.messages/);
  assert.doesNotMatch(sessionOutputViewerSource, /isSubagentEnvelopeMessage\(message\)/);
  assert.match(agentExecutionSource, /getDisplayableMessages\(messages, subagentProgress\.subagentMessageIndexes\)\.messages/);
  assert.doesNotMatch(agentExecutionSource, /isSubagentEnvelopeMessage\(message\)/);
  assert.doesNotMatch(agentExecutionSource, /'agentoutputtool'/);
  assert.doesNotMatch(sessionOutputViewerSource, /'agentoutputtool'/);
  assert.doesNotMatch(agentRunOutputViewerSource, /'agentoutputtool'/);
