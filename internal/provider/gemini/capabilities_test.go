...
func TestDiscoverProviderCapabilitiesLoadsGeminiFilesystemCapabilities(t *testing.T) {
	home := t.TempDir()
	projectPath := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)

	writeCapabilityFile(t, filepath.Join(home, ".gemini", "commands", "ask.md"), "Ask command")
	writeCapabilityFile(t, filepath.Join(home, ".gemini", "agents", "researcher.md"), "Research agent")
	writeCapabilityFile(t, filepath.Join(home, ".gemini", "skills", "video", "SKILL.md"), "---\ndescription: Video skill\n---\nSkill body")
	writeCapabilityFile(t, filepath.Join(projectPath, ".gemini", "skills", "project", "SKILL.md"), "Project skill")
