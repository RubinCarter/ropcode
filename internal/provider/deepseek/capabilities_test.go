...
func TestDiscoverProviderCapabilitiesLoadsDeepSeekFilesystemCapabilities(t *testing.T) {
	deepseekDir := t.TempDir()
	projectPath := t.TempDir()
	t.Setenv("DEEPSEEK_HOME", deepseekDir)

	writeCapabilityFile(t, filepath.Join(deepseekDir, "commands", "ask.md"), "Ask command")
	writeCapabilityFile(t, filepath.Join(deepseekDir, "agents", "researcher.md"), "Research agent")
	writeCapabilityFile(t, filepath.Join(deepseekDir, "skills", "video", "SKILL.md"), "---\ndescription: Video skill\n---\nSkill body")
	writeCapabilityFile(t, filepath.Join(projectPath, ".deepseek", "skills", "project", "SKILL.md"), "Project skill")
